run_cdhit <- function(protein_sequences) {
  ...
  for (i in 1:length(protein_sequences)) {
    cat(glue(">{names(protein_sequences)[i]}"), append = T, file = temp_fasta_file, "\n")
    cat(glue("{protein_sequences[i]}"), append = T, file = temp_fasta_file, "\n")
  }

  #now run cdhit: (obvs. need cdhit installed)
  system(glue("cdhit -i {temp_fasta_file} -o cdhit_output -d 100"))
  ...
  cdhit_clusters_output <- read.csv("cdhit_output.clstr", sep = "\t", row.names = NULL, header = FALSE, stringsAsFactors = FALSE) %>%
    ...
    mutate(cluster = ifelse(str_starts(cluster,">") == TRUE, cluster, NA),
    ...
    mutate(gene = str_split_fixed(str_split_fixed(gene, ">", 2)[,2],"[.][.][.]",2)[,1])
